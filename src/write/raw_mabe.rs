...
pub struct RawWriterMediumAlignedBigEndian;

impl RawWriterMediumAlignedBigEndian {
    pub fn write(writer: &mut UPBFWriter) -> Result<Vec<u8>, UPBFWriterError> {
        let mut out = Vec::new();

        out.write(b".UPBF\0")?;
        out.write(&u8::to_be_bytes(UPBFType::MediumAlignedBigEndian.into()))?;
        out.write(&u8::to_be_bytes(UPBFVersion::LAST_SUPPORTED.into()))?;
        out.write(&[0; 4])?; // FORMAT_ENTRY
        out.write(&[0; 4])?; // DATA_ENTRY
        out.write(&u32::to_be_bytes(usize_to_u32(writer.build_name.len(), UPBFWriterWriteError::InvalidBuildNameLength.into())?))?;
        out.write(&u32::to_be_bytes(usize_to_u32(writer.build_version.len(), UPBFWriterWriteError::InvalidBuildVersionLength.into())?))?;
        ...
        if !writer.data_formats.is_empty() {
            ...
            for format in &writer.data_formats {
                ...
                let block_start = u32::to_be_bytes(block_start);
                out[next_write_addr].copy_from_slice(&block_start);
                next_write_addr = last_offset..last_offset + 0x4; // NEXT

                out.write(&[0; 4])?; // NEXT
                out.write(&u32::to_be_bytes(usize_to_u32(format.name.len(), UPBFWriterWriteError::InvalidDataFormatNameLength.into())?))?;
                out.write(&u32::to_be_bytes(format.data_id))?;
        ...
        if !writer.data.is_empty() {
            ...
            for data in &writer.data {
                ...
                out.write(&[0; 4])?; // NEXT
                out.write(&u32::to_be_bytes(usize_to_u32(data.data.len(), UPBFWriterWriteError::InvalidDataLength.into())?))?;
                out.write(&u32::to_be_bytes(data.data_id))?;
                out.write(&u32::to_be_bytes(usize_to_u32(data.name.len(), UPBFWriterWriteError::InvalidDataNameLength.into())?))?;
