...
impl RawWriterMediumAlignedBigEndian {
    pub fn write(writer: &mut UPBFWriter) -> Result<Vec<u8>, UPBFWriterError> {
        ...
        out.write(&u32::to_be_bytes(usize_to_u32(writer.build_name.len(), UPBFWriterWriteError::InvalidNameLength.into())?))?;
        out.write(&u32::to_be_bytes(usize_to_u32(writer.build_version.len(), UPBFWriterWriteError::InvalidVersionLength.into())?))?;
        ...
        if !writer.data_formats.is_empty() {
            ...
            for format in &writer.data_formats {
                ...
                out.write(&[0; 4])?; // NEXT
                out.write(&u32::to_be_bytes(usize_to_u32(format.name.len(), UPBFWriterWriteError::InvalidFormatNameLength.into())?))?;
