...
pub struct RawWriterBigAlignedBigEndian;

impl RawWriterBigAlignedBigEndian {
    pub fn write(writer: &mut UPBFWriter) -> Result<Vec<u8>, UPBFWriterError> {
        let mut out = Vec::new();

        out.write(b".UPBF\0")?;
        out.write(&u8::to_be_bytes(UPBFType::BigAlignedBigEndian.into()))?;
        ...
        if !writer.data_formats.is_empty() {
            ...
            for format in &writer.data_formats {
                let block_start = u64::to_be_bytes(last_offset as u64);
        ...
        if !writer.data.is_empty() {
            ...
            for data in &writer.data {
                ...
                out.write(&[0; 8])?; // NEXT
                out.write(&u64::to_be_bytes(data.data.len() as u64))?;
