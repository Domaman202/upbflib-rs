...
impl RawReaderBigAlignedLittleEndian {
    pub fn read<'a>(reader: &'a UPBFReader, file_type: UPBFType, file_version: UPBFVersion) -> Result<UPBFReadResult<'a>, UPBFReaderError> {
        ...
        let build_name_len = u32::from_le_bytes(build_name_len) as usize;
        let build_name_len_aligned = align_len_big(build_name_len);
        if source.len() < 0x20 + build_name_len_aligned { return Err(UPBFReaderHeaderReadError::InvalidBuildNameLength.into()) }
        ...
        let build_version_len = u32::from_le_bytes(build_version_len) as usize;
        if source.len() < offset + 0x20 + align_len_big(build_version_len) { return Err(UPBFReaderHeaderReadError::InvalidBuildVersionLength.into()) }
        ...
        let mut offset = u64_to_usize(u64::from_le_bytes(offset), UPBFReaderDataReadError::InvalidOffset.into())?;
        let mut data_list = Vec::<UPBFDataForRead>::new();
        while offset != 0 {
            if source.len() < offset { return Err(UPBFReaderDataReadError::InvalidOffset.into()) }
            let next_offset = (&source[offset..offset + 0x8]).try_into();
            let next_offset = next_offset.map_err(|_| UPBFReaderError::InvalidFileLength)?;
            let next_offset = u64_to_usize(u64::from_le_bytes(next_offset), UPBFReaderDataReadError::InvalidOffset.into())?;
            let data_len = (&source[offset + 0x8..offset + 0x10]).try_into();
            let data_len = data_len.map_err(|_| UPBFReaderError::InvalidFileLength)?;
            let data_len = u64_to_usize(u64::from_le_bytes(data_len), UPBFReaderDataReadError::InvalidDataLength.into())?;
